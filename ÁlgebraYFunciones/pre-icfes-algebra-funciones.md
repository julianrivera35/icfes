...
**Ejemplo 1:** Simplificar $\dfrac{x^2 - 9}{x^2 + 5x + 6}$
...
**Ejercicio para practicar:** Factoriza y simplifica $\dfrac{2x^3 - 8x}{x^2 - 4}$. (Respuesta: $2x$, $x \neq \pm 2$)
...
$$a + b + c = 3, \quad a - b + c = 0, \quad c = 3 \quad \text{(sol: } y = x^2 + 2x + 3\text{)}$$
...
Oscilación: $\theta(t) = 0.3 \sin\!\left(\dfrac{2\pi t}{1.8}\right)$  
Periodo = 1.8 s (fórmula física $T=2\pi\sqrt{L/g} \approx 1.8$ s si $L \approx 0.8$ m)

**Ejercicio:** Hallar periodo de $y = 5 \cos(3x - \pi/2)$ (Respuesta: $\dfrac{2\pi}{3}$)
...
**Ejemplo:** Posición péndulo $x(t) = 0.3 \sin(3.5 t)$. Velocidad en $t=0$:

$$v(0) = 0.3 \cdot 3.5 \cdot \cos(0) = 1.05 \text{ m/s}$$

---

## 8. Magnitudes medias (velocidad media, aceleración media, densidad media)

$$v_m = \frac{\Delta x}{\Delta t} = \frac{f(b)-f(a)}{b-a} \qquad a_m = \frac{\Delta v}{\Delta t} \qquad \rho_m = \frac{m}{V}$$

**Ejemplo:** Densidad media de una barra de longitud 2 m con masa $m(x) = x^2 + 1$:

$$\rho_m = \frac{m(2)-m(0)}{2} = \frac{5}{2} = 2.5 \text{ kg/m}$$
...
| Cuadrática    | $\mathbb{R}$                                     | depende de $a$     | —          | Vértice $\left(-\frac{b}{2a}, \dots\right)$   |
...
## Ejercicios finales de síntesis

1. Factoriza y simplifica $\dfrac{x^4 - 16}{x^2 + 4x + 4}$.
